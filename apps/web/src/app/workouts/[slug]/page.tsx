...
export default function WorkoutPlanPage({
  ...
  const workoutPlan: WorkoutTemplate | undefined = findWorkoutBySlug(
    params.slug,
  );
  ...
  // подтягиваем выбранную дату для дневника
  useEffect(() => {
  ...
  const handleAddPlanToDiary = () => {
    ...
    const newWorkouts: DiaryWorkout[] = workoutPlan.exercises.map(
      (ex: WorkoutExerciseTemplate, idx: number) => ({
        ...
        // вес пользователь уже потом меняет в дневнике
      }),
    );
