...
import PageHeader from "@/app/components/PageHeader"; // путь подправь под себя
...
export default function ExercisePage({
  ...
  const data = useMemo(
    () => findExerciseWithPlan(params.slug),
    [params.slug],
  );
    ...
    <>
      ...
      <div className="relative z-10 min-h-screen bg-[var(--background)] text-[var(--text-primary)] pb-24 flex flex-col">
        ...
        <main className="px-3 pt-3 sm:px-4 md:px-5 flex flex-col gap-3">
          ...
          {exercise.techniqueSteps && exercise.techniqueSteps.length > 0 && (
            <motion.section
          ...
          {/* Небольшое место под заметки/дневник потом можно будет добавить */}
        </main>
      </div>
    </>
  );
